...
**Ponto de atenção 1**: os nomes de variáveis com caixa alta ou espaço entre elas. A função clean_names() será usada para ajustar os nomes.
**Ponto de atenção 2**: as características pessoais estão fora de ordem no banco de dados. A função relocate será usada para reordenar o banco de dados.
**Ponto de atenção 3**: a variável ID está com o nome de identidade. A função rename será usada para modificar o nome.
...
**Ajustes de acordo com pontos de atenção 1, 2 e 3**
```{r echo=FALSE}
names(bfi_fake)
```

## Passo 5 e 6: Checar tipos das variáveis/colunas e variável de id
...
## Passo 8: Checar respostas descuidadas
...
### Extra: Calcular respostas descuidadas via média e DP
...
# Considerações
- O banco de dados foi salvo na pasta data, seguindo as boas práticas de R.
